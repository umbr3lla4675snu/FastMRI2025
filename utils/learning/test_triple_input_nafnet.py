...
def visualize_triple_input_reconstruction(varnet_output, image_grappa, image_input, nafnet_output, fname, slice_idx, save_dir):
    """
    Visualize the triple input reconstruction process
    
    Args:
        varnet_output: VarNet reconstruction result
        image_grappa: GRAPPA reconstruction 
        image_input: Aliased input image
    ...
    input_img = image_input.cpu().numpy().squeeze()
    nafnet_img = nafnet_output.cpu().numpy().squeeze()
    
    # Create figure with 2x2 subplots
    fig, axes = plt.subplots(2, 2, figsize=(12, 12))
    fig.suptitle(f'Triple Input NAFNet Reconstruction\nFile: {fname}, Slice: {slice_idx}', fontsize=16)
    
    # Aliased Input Image (top-left)
    im1 = axes[0, 0].imshow(input_img, cmap='gray', vmin=0, vmax=input_img.max())
    axes[0, 0].set_title('Input (Aliased)', fontsize=14)
    axes[0, 0].axis('off')
    plt.colorbar(im1, ax=axes[0, 0], fraction=0.046, pad=0.04)
    
    # GRAPPA Reconstruction (top-right)
    im2 = axes[0, 1].imshow(grappa_img, cmap='gray', vmin=0, vmax=grappa_img.max())
    axes[0, 1].set_title('GRAPPA Reconstruction', fontsize=14)
    axes[0, 1].axis('off')
    plt.colorbar(im2, ax=axes[0, 1], fraction=0.046, pad=0.04)
    
    # VarNet Reconstruction (bottom-left)
    im3 = axes[1, 0].imshow(varnet_img, cmap='gray', vmin=0, vmax=varnet_img.max())
    axes[1, 0].set_title('VarNet Reconstruction', fontsize=14)
    axes[1, 0].axis('off')
    plt.colorbar(im3, ax=axes[1, 0], fraction=0.046, pad=0.04)
    
    # NAFNet Enhanced Result (bottom-right)
    im4 = axes[1, 1].imshow(nafnet_img, cmap='gray', vmin=0, vmax=nafnet_img.max())
    axes[1, 1].set_title('NAFNet Enhanced (Final)', fontsize=14)
    axes[1, 1].axis('off')
    ...
    save_path = os.path.join(save_dir, f'triple_input_visualization_{fname}_{slice_idx}.png')
...
def test(args, varnet_model, nafnet_model, data_loader):
    ...
    with torch.no_grad():
        for (mask, kspace, _, _, fnames, slices, _, image_grappa, image_input) in data_loader:
            ...
            # Forward pass through Triple Input NAFNet
            nafnet_output = nafnet_model(varnet_output, image_grappa, image_input)
            ...
            if sample_count == 0:
                ...
                visualize_triple_input_reconstruction(
                    varnet_output[0], 
                    image_grappa[0], 
                    image_input[0], 
                    nafnet_output[0],
...
def forward(args):
    ...
    nafnet_checkpoint = torch.load(args.exp_dir / 'best_model.pt', map_location=device, weights_only=False)
    ...
    forward_loader = create_triple_input_data_loaders(
