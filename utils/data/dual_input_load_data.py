...
class DualInputSliceData(Dataset):
    ...
    def __getitem__(self, i):
        ...
        with h5py.File(image_fname, "r") as hf:
            input_grappa = hf[self.input_key[1]][dataslice]  # Only GRAPPA, no input_alise needed
        if self.forward:
            target = -1
            attrs = -1
        ...
        return self.transform(mask, input_kspace, target, attrs, kspace_fname.name, dataslice, max_slice_index, input_grappa)


class DualInputDataTransform:
    ...
    def __call__(self, mask, input_kspace, target, attrs, fname, slice_idx, max_slice_index, grappa_image):
        def to_tensor(data):
        ...
        if not self.isforward:
            ...
            maximum = grappa_tensor.max().item()
            
            # Ensure maximum is reasonable
            if maximum < 0.1:
                maximum = 1.0
            
            # Even in forward mode, we need the actual images for dual input NAFNet
        ...
        return final_mask_tensor, kspace, target, maximum, fname, slice_idx, max_slice_index, grappa_image


def create_dual_input_data_loaders(data_path, args, shuffle=False, isforward=False):
    if isforward == False:
    ...
    data_storage = DualInputSliceData(
        root=data_path,
        transform=DualInputDataTransform(isforward, max_key_, args),
