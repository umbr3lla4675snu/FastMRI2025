"""
Dual Input NAFNet: NAFNet that takes 2 inputs for enhanced MRI reconstruction
...
class DualInputNAFNet(nn.Module):
    """
    NAFNet that takes two inputs (VarNet + GRAPPA) and produces enhanced reconstruction
    """
    def __init__(self, img_channel=1, width=32, middle_blk_num=8, 
                 enc_blk_nums=[2, 2, 4, 8], dec_blk_nums=[2, 2, 2, 2]):
        super().__init__()
        
        # Input channels = 2 (varnet + grappa)
        total_input_channels = 2 * img_channel
        
        # Initial convolution to process 2-channel input
    ...
    def forward(self, varnet_output, grappa_image):
        """
        Args:
        ...
        inp = torch.cat([varnet_output, grappa_image], dim=1)
...
class VarNetWithTripleInputNAFNet(nn.Module):
    ...
    def __init__(self, varnet_config=None, nafnet_config=None):
        ...
        self.nafnet = DualInputNAFNet(**nafnet_config)
        
    def forward(self, masked_kspace, mask, grappa_image):
        """
        Args:
        ...
        # Second stage: NAFNet enhancement (VarNet + GRAPPA)
        enhanced_output = self.nafnet(varnet_output, grappa_image)
        
        return enhanced_output


class DualInputNAFNetStandalone(nn.Module):
    """
    Standalone NAFNet for enhancing pre-reconstructed images (VarNet + GRAPPA)
    """
    def __init__(self, img_channel=1, width=32, middle_blk_num=8, 
                 enc_blk_nums=[2, 2, 4, 8], dec_blk_nums=[2, 2, 2, 2]):
        super().__init__()
        self.nafnet = DualInputNAFNet(
    ...
    def forward(self, varnet_output, grappa_image):
        ...
        """
        return self.nafnet(varnet_output, grappa_image)
